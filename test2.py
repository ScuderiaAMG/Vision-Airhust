...
#     return {
#         "TOTAL_TRAIN_HOURS": 8,
#         "BATCH_SIZE": min(int(gpu_mem * 15), 32),  # Reduced from 30 to 15
#         "GRAD_ACCUM_STEPS": 2,  # Added gradient accumulation
#         "NUM_WORKERS": min(cpu_cores * 4, logical_cores - 4),  
#         "MIXED_PRECISION": True,
#         "USE_CUDNN_BENCHMARK": True,
#         "MODEL": "resnet152",  
...
#         def safe_checkpoint(layer, segments, x):
#             child_count = len(list(layer.children()))
#             if segments <= 1 or child_count < segments:
#                 return layer(x)
#             return checkpoint_sequential(layer, segments, x, use_reentrant=False)
...
#     total_epochs = int(config['TOTAL_TRAIN_HOURS'] * 3600 / (len(train_loader) * batch_size))
#     scheduler = optim.lr_scheduler.OneCycleLR(
#         optimizer, 
#         max_lr=0.1,
#         steps_per_epoch=len(train_loader),
...
#     warmup_data = torch.rand(16, 3, 224, 224, device=device)  # Reduced warmup batch
#     for _ in range(10):  # Reduced warmup iterations
#         with torch.cuda.amp.autocast(enabled=config['MIXED_PRECISION']):
#             _ = model(warmup_data)
...
#                 print(f"[GPU {rank}] Epoch: {epoch} | Batch: {batch_idx}/{len(train_loader)} | "
#                       f"Loss: {loss.item() * accumulation_steps:.4f} | LR: {lr:.2e} | "
#                       f"GPU: {metrics['util']}% | Power: {metrics['power']:.1f}W | "
#                       f"Mem: {metrics['mem_used']:.1f}/{metrics['mem_total']:.1f}GB")
...
def get_hardware_config():
    ...
    return {
        "TOTAL_TRAIN_HOURS": 6,
        "BATCH_SIZE": min(int(gpu_mem * 15), 32),
        "GRAD_ACCUM_STEPS": 2,
        "NUM_WORKERS": min(cpu_cores * 4, logical_cores - 4),  
        "MIXED_PRECISION": True,
        "USE_CUDNN_BENCHMARK": True,
...
def train(rank, world_size):
    ...
    accumulation_steps = config['GRAD_ACCUM_STEPS']
    total_optimizer_steps = int(config['TOTAL_TRAIN_HOURS'] * 3600 * 1000)  # Large enough number
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer, 
        max_lr=0.1,
    ...
    warmup_data = torch.rand(16, 3, 224, 224, device=device)
    for _ in range(10):
        with torch.cuda.amp.autocast(enabled=config['MIXED_PRECISION']):
            _ = model(warmup_data)
    ...
    for epoch in range(10000):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % accumulation_steps == 0:
                ...
                if step_count < total_optimizer_steps:
                    scheduler.step()
                step_count += 1
...
def main():
    ...
    if checkpoint_files:
        ...
        print("No checkpoints found. Model not saved.")
